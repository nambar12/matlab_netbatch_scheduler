...
if ~isprop(cluster.AdditionalProperties, 'ClusterHost')
    error('parallelexamples:GenericNetbatch:MissingAdditionalProperties', ...
...
if isempty(cluster.UserData)
    ...
    if ~isstruct(cluster.UserData)
        error('parallelexamples:GenericNetbatch:IncorrectUserData', ...
    ...
    if isfield(cluster.UserData, 'RemoteConnection')
        ...
        if isempty(remoteConnection)
            ...
            elseif ~strcmpi(remoteConnection.Hostname, clusterHost)
                ...
                warning('parallelexamples:GenericNetbatch:DifferentRemoteParameters', ...
...
if isprop(cluster.AdditionalProperties, 'AuthenticationMode')
    ...
    if ~ischar(authMode) || ~ismember(authMode, {'Agent', 'IdentityFile', 'Multifactor', 'Password'})
        error('parallelexamples:GenericNetbatch:IncorrectArguments', ...
            'AuthenticationMode must be one of ''Agent'', ''IdentityFile'', ''Multifactor'' or ''Password''');
    end
...
if useUI
    ...
    if isempty(usernameResponse)
        % User hit cancel
        error('parallelexamples:GenericNetbatch:UserCancelledOperation', ...
...
if useUI
    ...
    if strcmp(passphraseResponse, 'Cancel')
        % User hit cancel
        error('parallelexamples:GenericNetbatch:UserCancelledOperation', 'User cancelled operation.');
    end
